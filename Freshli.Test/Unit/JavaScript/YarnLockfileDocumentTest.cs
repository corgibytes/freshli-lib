...
namespace Freshli.Test.Unit.JavaScript {
  public class YarnLockfileDocumentTest {
    ...
    public void ReadSimpleDocumentWithoutEnumerator() {
      ...
      Assert.Equal("version", ansiRegex.GetProperty("version").Name);
      Assert.Equal("5.0.0", ansiRegex.GetProperty("version").Value);

      var normalizeUrl = root.GetProperty("normalize-url@^5.1.0");
      Assert.Equal("normalize-url@^5.1.0", normalizeUrl.Name);
      Assert.Equal("version", normalizeUrl.GetProperty("version").Name);
      Assert.Equal("5.1.0", normalizeUrl.GetProperty("version").Value);

      var stripAnsi = root.GetProperty("strip-ansi@^6.0.0");
      Assert.Equal("strip-ansi@^6.0.0", stripAnsi.Name);
      Assert.Equal("version", stripAnsi.GetProperty("version").Name);
      Assert.Equal("6.0.0", stripAnsi.GetProperty("version").Value);
    }
